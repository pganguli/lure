...
class Plotter:
    ...
    def plot_mlife_plot(
        ...
        ax.text(
            x=metric_meas_period[-1] * 0.7,
            y=avg_steady_state_error + (0.035 * avg_steady_state_error),
            s=f"Avg:{avg_steady_state_error}",
            fontsize=10,
        )
        ax.axhline(y=max_steady_state_error, xmin=0.75, color="g", linestyle="--")
        ax.text(
            x=metric_meas_period[-1] * 0.7,
            y=max_steady_state_error + (0.01 * max_steady_state_error),
            s=f"Max:{max_steady_state_error}",
            fontsize=10,
        )
        ax.axhline(y=min_steady_state_error, xmin=0.75, color="g", linestyle="--")
        ax.text(
            x=metric_meas_period[-1] * 0.7,
            y=min_steady_state_error + (0.05 * min_steady_state_error),
            s=f"Min:{min_steady_state_error}",
            fontsize=10,
        )
        ...
        if print_pdf:
            ...
            while True:
                ...
                if (
                    not os.path.isfile(
                        f"{self.output_dir}/mlife_errors_subplot_ser{ser}_trial_{trial}.pdf"
                    )
                ):
                    plt.savefig(
                        f"{self.output_dir}/mlife_errors_subplot_ser{ser}_trial_{trial}.pdf",
    ...
    def plot_xseries_vs_sst(self, med_error, cis, labels, x_vals, print_pdf=True):
        ...
        for config in range(np.shape(med_error)[0]):
            ...
            line = ax.plot(
                x_vals,
                median_vals,
                linestyle=self.linestyles[config],
                marker=self.markers[config],
                markersize=7,
                label=labels[config],
            ...
            ax.fill_between(
                x_vals, lower_bound, upper_bound, color=line[0].get_color(), alpha=0.3
            )
        ...
        if print_pdf:
            ...
            while True:
                n = n + 1
                if not os.path.isfile(f"lcr_error_{n}.pdf"):
