...
class UtilityClass:
    ...
    def unpickle_data(pickled_file_name):
        if not os.path.isfile(f"{pickled_file_name}"):
...
class Analyzer(UtilityClass):
    ...
    def initialize(self):
        ...
        (
            self.x_values,
            self.node_boot_events,
            self.nodes_die_events,
            self.nodes_succ_comm_events,
        ...
        self.met_meas_period = int(
            self.sim_time / 20
        )  # ms creates 20 measurement period
        ...
        for exp_result in self.lure_results:
            for series_result in exp_result:
                for x_value, sim_results in series_result.items():
                    ...
                    time_tm.append([])
                    print(
                        f"Loading time series data for {series_result.key}, xval {x_value}..."
                    )
                    ...
                    for sim_result in sim_results:
                        ...
                        time_tm[-1].append([])
                        for n in sim_result:
                            restart_events[-1][-1].append(
                                np.asarray(
                                    n.get_time_series(StatType.NODE_RESTART),
                                    dtype=object,
                                )
                            )
                            die_events[-1][-1].append(
                                np.asarray(
                                    n.get_time_series(StatType.NODE_DIE), dtype=object
                                )
                            )
                            succ_comm_events[-1][-1].append(
                                np.asarray(
                                    n.get_time_series(StatType.SUCC_COMM), dtype=object
                                )
                            )
                            time_tm[-1][-1].append(
                                np.asarray(
                                    n.get_time_series("est_shared_time_tm"),
                                    dtype=object,
                                )
                            )
        return (
            np.asarray(x_values, dtype=object),
            np.asarray(restart_events, dtype=object),
            np.asarray(die_events, dtype=object),
            np.asarray(succ_comm_events, dtype=object),
            np.asarray(time_tm, dtype=object),
        )
    ...
    def run_metrics_analysis(self, pickle_data=False):
        ...
        for node_series in range(self.num_series):
            for trial in range(self.num_trials):
                self.metric_analyzer.init_params(
                    self.num_nodes,
                    self.node_boot_events[node_series, trial, :],
                    self.nodes_die_events[node_series, trial, :],
                    self.nodes_succ_comm_events[node_series, trial, :],
                    self.nodes_time_tm10[node_series, trial, :],
                ...
                (
                    ...
                    _,
                    _,
                ) = self.metric_analyzer.run_metrics_calc()
        ...
        if pickle_data:
            self.pickle_data(
                m_life_errors,
                f"{self.sim_output_dir}/pickled_data/errors_{self.sim_output_dir}",
            )
...
class AggDataProcessor(UtilityClass):
    ...
    def calc_error_stats(self, ci_coeff=1.96):
        ...
        for dir in self.sim_out_dirs:
            data_error = np.array(
                self.unpickle_data(f"{dir}/pickled_data/errors_{dir}")
            )
            cis = []
            median_vals = []
            for ser in range(np.shape(data_error)[0]):
                ...
                cis.append(
                    ci_coeff * np.std(sse_avg, axis=0) / np.sqrt(np.shape(sse_avg)[0])
                )
...
def main():
    ...
    parser.add_argument(
        "-d", "--outdirs", nargs="+", help="A list of output directories"
    )
    parser.add_argument("-x", "--xvals", nargs="+", help="A list of lcr xvalues")
    parser.add_argument(
        "-c", "--configs", nargs="+", help="A list of lmp configs directories"
    )
    parser.add_argument(
        "-p",
        "--plot_only",
        action="store_true",
        help="Plots from cached processed results",
    )
    parser.add_argument(
        "-i",
        "--x_indices",
        nargs="+",
        type=int,
        help="List of x-value indices to plot (defaults to all)",
    )
    # Parse the arguments
