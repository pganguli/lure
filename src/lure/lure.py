...
from lure.results import (
    LURE_RESULTS_FILENAME,
    ExperimentResult,
    LureResults,
    SeriesResult,
)
...
class Lure:
                                ...
                                for x_value in metadata.x_values:
                                    ...
                                    try:
                                        with os.scandir(x_path) as x_it:
                                            sim_entries = sorted(
                                                x_it, key=lambda e: e.name
                                            )
                                            for sim_entry in sim_entries:
                                                if sim_entry.is_dir():
                                                    series_result.add_simulation(
                                                        x_value, int(sim_entry.name)
                                                    )
    ...
    def load_results(cls, output_dir: str = "output") -> LureResults:
        ...
        try:
            ...
            print(
                "Info: LureResults file not found, attempting conversion from older results format."
            )
    ...
    def __init__(
        ...
        if not resume:
            try:
                shutil.move(
                    self.output_dir,
                    f"{self.output_dir}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
                )
            except FileNotFoundError:
                pass
        ...
        if self.email_notify:
            try:
                ...
                print(
                    "EMAIL CONFIG ERROR: Email configuration item was not found or improperly formatted"
                )
                raise
        ...
        if self.num_procs > 1:
            with Pool(self.num_procs) as p:
                for i, exp in enumerate(self.lure_config.experiments):
                    ...
                    total_exp_steps = len(experiment.series) * len(
                        experiment.series[0].x_values
                    )  # total = series * x_vals
                    if self.progress_bar:
                        with alive_bar(
                            total_exp_steps, title=f"Experiment {i + 1}/{exp_total}"
                        ) as bar:
                            self.results.add_experiment(
                                experiment.run(p, progress_bar=bar)
                            )
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-C", type=str, default="config", help="Configuration directory."
    )
    parser.add_argument(
        "-c",
        type=str,
        default=None,
        help="Top-level configuration file (e.g. lifecycle_ratio.json).",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Keep existing results and run any trials without existing results.",
    )
    args = parser.parse_args()

    simulator = Lure(
        config_dir=args.C, top_config_file=args.c, output_dir=args.o, resume=args.resume
    )
    simulator.run()
