...
class GreedyClockILL(BasicILL):
    ...
    def initialize(self, node: "SensorNode"):
        ...
        self.proposed_ttr = 0
        self.cutoff_ontime = (
            self.power_supply.get_max_ontime_energy()
            / self.operating_discharge_rate
            * 0.75
        )
        self.their_ttr = 0
    ...
    def _time_to_rendezvous(self, time_til_off):
        current_energy = self.power_supply.get_current_energy() - min(
            time_til_off,
            self.power_supply.get_time_to_death(self.operating_discharge_rate),
        ) * (self.operating_discharge_rate - self.harvester.get_next_charging_power())
        self.storage.get_energy_for_voltage(
            self.power_supply.threshold_to_die_v
        )
        energy_at_on = self.storage.get_energy_for_voltage(
            self.power_supply.threshold_to_restart_v
        )
        ...
        if current_energy >= energy_at_on:
            ...
            time_off = max(
                (energy_at_on - current_energy)
                / self.harvester.get_next_charging_power(),
                0,
            )

        time_to_rendezvous = time_off + self.boot_time + 0.000001
        self.debug(
            f"time_to_rendezvous {time_to_rendezvous}, harvester {self.harvester.get_next_charging_power()}"
        )
    ...
    def frame(self, packet: Packet) -> bool:
        ...
        if packet.type is PacketType.ACK:
            ...
            if my_ttr >= 0 and self.their_ttr >= 0:
                ...
                self.debug(
                    f"Configuring LMP time to rendezvous to {time_to_rendezvous} (clock tick {self.time_module.time() + time_to_rendezvous})"
                )
                packet.set_header(self.HEADER_KEY_RENDEZVOUS, time_to_rendezvous, 4)
                self.lmp.set_config(
                    LMPConfigKey.OTHER, self.time_module.time() + time_to_rendezvous
                )
                ...
                if self.their_last_call or last_call:
                    ...
                    self.debug(
                        f"Locking in LMP. their_last_call {self.their_last_call}, mine {last_call}"
                    )
                else:
                ...
                self.debug(
                    f"Could not select a TTR (comparing {my_ttr} and {self.their_ttr})"
                )
                return False
        else:
...
class OracleClockILL(GreedyClockILL, EnergyModelObserver):
    ...
    def initialize(self, node: "SensorNode"):
        super().initialize(node)
        self.other_nodes = [
            n for n in node.simulation.nodes if n.node_id != node.node_id
        ]
    ...
    def on_update(self):
        if self.lmp.get_config(LMPConfigKey.OTHER) is not None:
            ...
            if my_ttr >= 0 and their_ttr >= 0:
                ...
                self.lmp.set_config(
                    LMPConfigKey.OTHER, new_ttr + self.time_module.time()
                )
