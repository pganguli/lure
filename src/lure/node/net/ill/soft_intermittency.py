...
class SIGreedyILL(BasicILL):
    ...
    def _time_to_rendezvous(self):
        ...
        energy_at_sleep = self.storage.get_energy_for_voltage(
            self.lmp.get_config(LMPConfigKey.SLEEP_THRESHOLD)
        )
        energy_at_wake = self.storage.get_energy_for_voltage(
            self.lmp.get_config(LMPConfigKey.WAKE_THRESHOLD)
        )
        # Need to use get_next_charging_power() here, not harvester.charging_power
        if self._node.state is NodeState.OPERATING:
            time_til_sleep = max(
                (current_energy - energy_at_sleep)
                / (
        ...
        if self._node.state is NodeState.SLEEPING:
            if current_energy >= energy_at_wake:
                ...
                time_asleep = max(
                    (energy_at_wake - current_energy)
                    / (
                        self.harvester.get_next_charging_power()
                        - self.sleeping_discharge_rate
                    ),
                    0,
                )
        else:
            time_asleep = max(
                (energy_at_wake - energy_at_sleep)
                / (
                    self.harvester.get_next_charging_power()
                    - self.sleeping_discharge_rate
                ),
        ...
        self.debug(
            f"time_til_sleep {time_til_sleep}, time_asleep {time_asleep}, time_to_rendezvous {time_to_rendezvous}, harvester {self.harvester.charging_power}, discharge {self.sleeping_discharge_rate}"
        )
    ...
    def parse(self, packet: Packet) -> bool:
        ...
        if my_ttr >= 0 and their_ttr >= 0:
            ...
            self.debug(
                f"Configuring LMP time to rendezvous to {time_to_rendezvous} (clock tick {self.time_module.clock() + time_to_rendezvous})"
            )
            self.lmp.set_config(
                LMPConfigKey.OTHER, self.time_module.clock() + time_to_rendezvous
            )
        else:
    ...
    def on_update(self):
        if self.lmp.get_config(LMPConfigKey.OTHER) is not None:
            ...
            if my_ttr >= 0 and their_ttr >= 0:
                ...
                self.lmp.set_config(
                    LMPConfigKey.OTHER, new_ttr + self.time_module.clock()
                )
