...
class GILL(BasicILL):
    ...
    def boot(self):
        ...
        if self.send_queue.queues_empty():
            self.lmp.set_config(
                LMPConfigKey.ON_TIME_MS,
                self.receiver_on_slots * self.netstack.slot_length,
            )
            self.stats.increment("ill_boot_as_receiver")
            self.debug("entered receive mode on boot")
        else:
            self.lmp.set_config(
                LMPConfigKey.ON_TIME_MS,
                self.sender_on_slots * self.netstack.slot_length,
            )
            self.stats.increment("ill_boot_as_sender")
            self.debug("entered sender mode on boot")
    ...
    def send_packet(self, packet: Packet, booting: bool = False) -> bool:
        ...
        if ret:
            ...
            self.debug(
                "node:, ill: find mac successfully start sending. Set LMP to sender on-time in send_packet function"
            )
            self.debug("entered sender mode (send_packet)")
    ...
    def packet_sent(self, packet: Packet, success: bool):
        ...
        if self.send_queue.queues_empty():
            ...
            self.lmp.set_config(
                LMPConfigKey.ON_TIME_MS,
                self.receiver_on_slots * self.netstack.slot_length
                + self.time_module.clock(),
            )
            self.stats.increment("ill_switch_to_receiver")
            self.debug("entered receive mode, queue empty")
