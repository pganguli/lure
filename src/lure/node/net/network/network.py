...
class Network(StatsProvider, Loggable):
    ...
    def send_packet(self, destination_id: int, payload: str):
        ...
        self.debug(
            f"Packet passed to ILL. dest={pkt.destination_id}, next_hop: {pkt.next_hop}"
        )
    ...
    def packet_received(self, packet: Packet, sender_id: int):
        ...
        if packet.destination_id == self.node_id:
            ...
            self.stats.time_series_append(
                StatType.NETWORK_PACKET_TRAILS, packet.net_record
            )
            self.application_receive_cb(packet, sender_id)
        else:
            if (packet.destination_id is None) and (
                packet.next_hop == self.netstack.BROADCAST_ADDRESS
            ):
                self.debug("Broadcast packet received")
                self.stats.time_series_append(
                    StatType.NETWORK_BROADCASTS,
                    (
                        sender_id,
                        self.netstack.physical.get_neighbor_by_id(
                            sender_id
                        ).physical.get_neighbors(),
                    ),
                )
                self.application_receive_cb(packet, sender_id)
            else:
                self.debug(
                    f"Not this packet's destination. Next hop is {packet.next_hop}"
                )
    ...
    def get_next_hop(self, packet: Packet) -> int:
        ...
        if packet.destination_id is None:
            raise ValueError(
                "Packet submitted to network.get_next_hop() must have a destination_id"
            )
        else:
            return None
