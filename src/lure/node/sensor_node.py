...
class SensorNode(Loggable, StatsProvider):
    ...
    def __init__(self, config: SensorNodeConfig):
        ...
        self.time_module: TimeModule = Config.instantiate_from_dict(
            config.config["time_module"], "lure.node.time"
        )
        self.power_supply: PowerSupply = Config.instantiate_from_dict(
            config.config["power_supply"], "lure.node.power"
        )
        self.lmp: LMP = Config.instantiate_from_dict(
            config.config["lmp"], "lure.node.lmp"
        )
        self.netstack: Netstack = Config.instantiate_from_dict(
            config.config["netstack"], "lure.node.net"
        )
        self.netstack.set_addr(self.node_id)
        self.traffic_generator: TrafficGenerator = Config.instantiate_from_dict(
            config.config["traffic_generator"], "lure.node.traffic"
        )
    ...
    def init_with_simulation(self, sim: "Simulation", logger: LureLogger):
        ...
        self.timestepper.register_log(
            logger,
            self.logger_config.get_level("node"),
            f"{node_tag}{logger.sep}Timestepper",
        )
        self.lmp.register_log(
            logger, self.logger_config.get_level("lmp"), f"{node_tag}{logger.sep}LMP"
        )
        self.netstack.mac.register_log(
            logger, self.logger_config.get_level("mac"), f"{node_tag}{logger.sep}MAC"
        )
        self.netstack.ill.register_log(
            logger, self.logger_config.get_level("ill"), f"{node_tag}{logger.sep}ILL"
        )
        self.netstack.network.register_log(
            logger,
            self.logger_config.get_level("network"),
            f"{node_tag}{logger.sep}NETWK",
        )
        self.netstack.physical.register_log(
            logger,
            self.logger_config.get_level("physical"),
            f"{node_tag}{logger.sep}PHYS",
        )
        self.netstack.ill.send_queue.register_log(
            logger,
            self.logger_config.get_level("queue"),
            f"{node_tag}{logger.sep}PacketQueue",
        )
        self.power_supply.register_log(
            logger,
            self.logger_config.get_level("power_supply"),
            f"{node_tag}{logger.sep}PowerSupply",
        )
        self.power_supply.storage.register_log(
            logger,
            self.logger_config.get_level("power_supply"),
            f"{node_tag}{logger.sep}Storage",
        )
        self.power_supply.harvester.register_log(
            logger,
            self.logger_config.get_level("power_supply"),
            f"{node_tag}{logger.sep}Harvester",
        )
        self.traffic_generator.register_log(
            logger,
            self.logger_config.get_level("traffic"),
            f"{node_tag}{logger.sep}TrafficGenerator",
        )
        self.time_module.register_log(
            logger,
            self.logger_config.get_level("time"),
            f"{node_tag}{logger.sep}TimeModule",
        )
        ...
        if (
            self.power_supply.get_time_to_restart() is not None
            and self.power_supply.get_time_to_restart() <= 0
        ):
            self.restart()
        else:
            self.state = NodeState.OFF
        self.lmp.initialize(self)
        self.netstack.initialize(
            self, [n.netstack for n in sim.nodes if n is not self], sim.simpy_env
        )
    ...
    def get_avg_discharge_power_w(self):
        ...
        max_ontime = (
            Eon
            - self.os_boot_time_ms
            * (
                self.discharge_rates[NodeState.BOOTING]
                - self.discharge_rates[NodeState.OPERATING]
            )
        ) / self.discharge_rates[NodeState.OPERATING]
        boot_fraction = (
            self.os_boot_time_ms / max_ontime
        )  # (self.os_boot_time_ms + self.lmp.get_avg_ontime_ms(max_ontime - self.os_boot_time_ms))

        return self.discharge_rates[
            NodeState.BOOTING
        ] * boot_fraction + self.discharge_rates[NodeState.OPERATING] * (
            1 - boot_fraction
        )
    ...
    def execute(self, simpy_env: simpy.core.Environment, energy_model: EnergyModel):
        ...
        while True:
            ...
            above_minimum_voltage = self.power_supply.execute(
                self.discharge_rates[self.state]
            )
            ...
            if self.state is NodeState.OFF:
                ...
                if time_to_restart is not None:
                    if time_to_restart <= 0:
                        ...
                        self.debug("Not time to restart yet.")

            # Consume!
            else:
                if not above_minimum_voltage:
                    self.die()
                    self.info("Dies due to no energy")
                    ...
                    if self._perfect_clock >= self.os_boot_time_ms:
                        ...
                        if self.state is NodeState.BOOTING:  # + RADIO_BOOTUP_TIME:
                            ...
                            if self.last_off_time > 0:
                                last_off_duration = (
                                    self.last_on_time - self.last_off_time
                                )
                                self.stats.time_series_append(
                                    StatType.NODE_OFFTIME, last_off_duration
                                )
                                self.debug(
                                    f" Node {self.node_id} was off for {last_off_duration} ms"
                                )
                            else:
                                last_off_duration = 0

                            self.time_module.boot(
                                last_off_duration + self.os_boot_time_ms
                            )
                            self.lmp.boot()
                            self.netstack.boot()
                            self.netstack.network.register_receive_cb(
                                self.app_receive_callback
                            )
                            self.netstack.network.register_sent_cb(
                                self.app_sent_callback
                            )
                            ...
                            for node in [
                                n
                                for n in self.simulation.nodes
                                ...
                                if node.state is NodeState.OPERATING:
                                    ...
                                    self.debug(
                                        f"Overlap -- self: {self.on_neighbors}, node: {node.on_neighbors}"
                                    )
                        # else:
                        ...
                        if self.state is NodeState.OPERATING:
                            ...
                            if num_generated_pkts > 0:
                                ...
                                for pkt in range(num_generated_pkts):
                                    ...
                                    self.netstack.network.send_packet(
                        ...
                        if (
                            self.state is NodeState.OPERATING
                            or self.state is NodeState.SLEEPING
                        ):
                            next_state = self.lmp.execute()
                            if next_state is NodeState.OFF:
                                self.die()
                                self.info("Dies due to LMP enable")
                            elif (
                                self.state is NodeState.OPERATING
                                and next_state is NodeState.SLEEPING
                            ):
                                self.sleep()
                            elif (
                                self.state is NodeState.SLEEPING
                                and next_state is NodeState.OPERATING
                            ):
                                self.wake()
    ...
    def app_receive_callback(self, packet: Packet, sender_id: int):
        ...
        """
        self.debug(
            f"APP|Packet received from network, source node: {sender_id}, destination: {packet.destination_id}, payload: {packet.payload}"
        )
        self.info("Packet received")
    ...
    def app_sent_callback(self, packet: Packet, status: bool):
        ...
        """
        self.debug(
            f"APP|Sent packet: receiving node: {packet.next_hop}, status: {status}"
        )
        if status:
    ...
    def set_state_timer(self):
        ...
        if self.state is NodeState.BOOTING:
            ...
            if time_to_next is None or time_to_next <= 0:
                time_to_next = self.power_supply.get_time_to_death(
                    self.discharge_rates[NodeState.SLEEPING]
                )
