...
from lure.node.time.time import NodeTime
...
class OracleEarlyDieClockLMP(FixedLMP):
    ...
    def initialize(self, node: "SensorNode"):
        ...
        self.max_on_time = (
            node.power_supply.get_max_ontime_energy()
            / node.discharge_rates[NodeState.OPERATING]
            - node.os_boot_time_ms
        )
    ...
    def _early_die_for_rendezvous(self):
        ...
        if (
            curr_time + (self.max_on_time - self.time_module.clock())
            ...
            max_offtime = self.power_supply.harvester.get_time_to_harvest(
            ...
            if curr_time + min_offtime + self.boot_time >= self.rendezvous_time:
                self.debug(
                    f"Cannot make rendezvous {self.rendezvous_time}... min offtime {min_offtime}"
                )
                self.set_config(LMPConfigKey.ON_TIME_MS, self.time_module.clock())

            else:
                ttr_from_boot = self.rendezvous_time - (
                    curr_time - self.time_module.clock()
                )
                ...
                Ton = (
                    lifecycle_period * Pc
                    - self.boot_time * self.discharge_rates[NodeState.BOOTING]
                ) / self.discharge_rates[NodeState.OPERATING]
                self.debug(
                    f"Setting early die at {Ton} for rendezvous at {self.rendezvous_time} (planning on {num_lifecycles} lifecycles of {lifecycle_period})"
                )
                if Ton >= self.time_module.clock():
                    ...
                    self.debug(
                        f"Ton already passed... {Ton} vs. {self.time_module.clock()}"
                    )
