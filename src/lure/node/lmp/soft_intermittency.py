...
class SoftIntermittencyLMP(LMP):
    ...
    def execute(self) -> NodeState:
        if self.power_supply.storage.voltage <= self.sleep_threshold_v:
            ...
            self.sleeping and self.power_supply.storage.voltage >= self.wake_threshold_v
        ):
            self.sleeping = False
    ...
    def get_time_til_wake(self) -> SimTime:
        ...
        rate = (
            self.power_supply.harvester.charging_power - self.sleeping_discharge_rate_w
        )
        if rate == 0:
            return None

        return (
            self.power_supply.storage.get_energy_for_voltage(self.wake_threshold_v)
            - self.power_supply.get_current_energy()
        ) / rate
    ...
    def get_time_til_sleep(self) -> SimTime:
        ...
        rate = (
            self.operating_discharge_rate_w - self.power_supply.harvester.charging_power
        )
        return (
            self.power_supply.get_current_energy()
            - self.power_supply.storage.get_energy_for_voltage(self.sleep_threshold_v)
        ) / rate

    def __str__(self):
        return "SoftIntermittencyLMP"
...
class SIClockLMP(SoftIntermittencyLMP):
        ...
        if key is LMPConfigKey.OTHER:
            ...
            try:
                ...
                self.time_module.set_relative_timer(
                    self.TIMER_KEY_WAKE, self.wake_time - self.time_module.clock()
                )
                return True
            except ValueError:
                self.debug(f"Error: {value} not valid for {key} in LMP")
    ...
    def execute(self) -> NodeState:
        self.debug(
            f"wake_time {self.wake_time}, Timer expired {self.time_module.timer_expired(self.TIMER_KEY_WAKE)}, voltage {self.power_supply.storage.voltage}, wake_threshold {self.wake_threshold_v}"
        )
        if self.power_supply.storage.voltage <= self.sleep_threshold_v:
            self.sleeping = True
        elif (
            (self.sleeping)
            and self.power_supply.storage.voltage >= self.wake_threshold_v
            and (
                self.wake_time is None
                or self.time_module.timer_expired(self.TIMER_KEY_WAKE)
            )
        ):
            self.sleeping = False
            self.wake_time = None
