...
class PersistentClock(Clock, ABC):
    def __init__(self, config: Config):
        ...
        self.exp_data = self.unpickle_data(
            os.path.dirname(os.path.realpath(__file__)) + "/pclk_exp_data.pickle"
        )
        # Create map tables
        ...
        for channel in range(self.num_channels):
            for off_time_idx in range(num_off_times):
                ...
                if mean_map_table[channel][:, 0][off_time_idx] < discharge_volts:
                    volt_discharged.append(
                        [
                            channel,
                            mean_map_table[channel][:, 0][off_time_idx],
                            off_time_idx,
                        ]
                    )
    ...
    def findTimeAndSlope(self, time_arr, volt_arr, input_volts):
        ...
        if input_volts > volt_arr[0]:
            pred_time = self.linear_extrapolation(
                volt_arr[0], time_arr[0], volt_arr[1], time_arr[1], input_volts
            )
            ...
            td = time_arr[
                0
            ]  # time difference between the mapping table times for interpolation/bin size
        ...
        # If time exactly matches in one of the entry of mapping table
            ...
            if input_volts == volt_arr[-1]:
                ...
                slope = self.find_slope(
                    volt_arr[matched_idx],
                    time_arr[matched_idx],
                    volt_arr[matched_idx - 1],
                    time_arr[matched_idx - 1],
                )
            ts_pos_diff = 0
            ts_neg_diff = 0
        # If input voltage is in between then do interpolation
            ...
            try:
                ...
                pred_time = np.interp(
                    input_volts,
                    [volt_arr[idx], volt_arr[idx + 1]],
                    [time_arr[idx], time_arr[idx + 1]],
                )
                slope = self.find_slope(
                    volt_arr[idx], time_arr[idx], volt_arr[idx + 1], time_arr[idx + 1]
                )
        ...
        for exp in range(num_exp):
            for off_time_idx in range(num_off_times):
                for channel in range(num_channels):
                    ...
                    est_time, _, td, ts_pos_diff, ts_neg_diff = self.findTimeAndSlope(
                        time_arr, volt_arr, voltage
                    )
    ...
    def get_volt_from_time(self, time_arr, volt_arr, off_time):
        ...
        if off_time < time_arr[0]:
            est_voltage = self.linear_extra(
                time_arr[0], volt_arr[0], time_arr[1], volt_arr[1], off_time
            )
    ...
    def get_channel_voltages(self, channel):
        ...
        return (
            np.asarray(sorted_channel_voltages, dtype=object),
            np.asarray(sorted_channel_times, dtype=object),
            max_measured_voltage,
        )
    ...
    def cal_est_time_from_offtime(self, input_offtimes, clk_type):
        ...
        for off_time in input_offtimes:
            ...
            for channel in range(self.num_channels):
                ...
                channel_voltages, channel_times, max_meas_voltage = (
                    self.get_channel_voltages(channel)
                )
                est_voltage = self.get_volt_from_time(
                    channel_times, channel_voltages, off_time
                )
                pred_time, slope, td, tsPosDiff, tsNegDiff = self.findTimeAndSlope(
                    time_arr, volt_arr, est_voltage
                )
            ...
            hlite_max_meas_allchannels_allofftimes.append(
                np.asarray(hlite_max_meas_channels, dtype=object)
            )
            est_time_channels = deque([])
            channels_slope = deque([])
            hlite_max_meas_channels = []
        ...
        for est_time in est_time_ip_offtimes:
            for channel, time in enumerate(est_time):
                if (
                    time
                    >= np.round(self.map_tables[channel][1], decimals=6)[
                        discharge_idx_channel[channel] - 1
                    ]
                ):
            ...
            max_meas_allchannels_allofftimes.append(
                np.asarray(max_meas_channels, dtype=object)
            )
            max_meas_channels = []

        max_meas_allchannels_allofftimes = np.asarray(
            max_meas_allchannels_allofftimes, dtype=object
        )

        return (
            est_time_ip_offtimes,
            sel_est_times,
            max_meas_allchannels_allofftimes,
            hlite_max_meas_allchannels_allofftimes,
            max_meas_time_hlite,
