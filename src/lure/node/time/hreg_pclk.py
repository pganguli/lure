...
class HarcRegPersistentClock(PersistentClock):
    ...
    def get_est_offtimes(self, input_off_time):
        ...
        pred_batches, testDataSVMScaled = self.predict_batch(
            svm_classifier_model=svm_classifier,
            scaler=svm_scaler,
            test_data=comp_clk_est_time,
        )
        # Estimate off times using trained lasso model
        est_off_times, max_meas_list, max_time_meas = self.get_est_time_hreg(
            trained_lasso_model, comp_clk_est_time, pred_batches
        )
        if self.time_units == "ms":
    ...
    def get_trained_SVM_model(self, est_time_exp_train, num_exp_train):
        ...
        train_data = np.reshape(
            est_time_exp_train,
            (
                np.shape(est_time_exp_train)[0] * np.shape(est_time_exp_train)[1],
                np.shape(est_time_exp_train)[2],
            ),
        )
        exp_data_batches = np.asarray(exp_data_batches, dtype=object)
    ...
    def get_trained_lassoreg_model(
        self, exp_data_train, est_time_exp_train, scale_data_flag=False
    ):
        """
        ...
        for n_batch in self.batch:
            ...
            all_exp_batch_estTimes = np.reshape(
                all_exp_batch_estTimes,
                (
                    np.shape(all_exp_batch_estTimes)[0]
                    * np.shape(all_exp_batch_estTimes)[1],
                    np.shape(all_exp_batch_estTimes)[2],
                ),
            )
            ...
            if scale_data_flag:
                model_aic = make_pipeline(
                    StandardScaler(),
                    linear_model.LassoLarsIC(criterion="aic", max_iter=100000),
                )
    ...
    def get_trained_models(self):
        ...
        svm_classifier, svm_scaler = self.get_trained_SVM_model(
            est_time_exp_train, num_exp_train
        )
        # Get trained lasso regression model
        trained_lasso_model, lassoScaler = self.get_trained_lassoreg_model(
            exp_data_train, est_time_exp_train
        )
    ...
    def get_est_time_hreg(self, trained_model, comp_clk_est_time, predicted_batches):
        ...
        max_time_meas = trained_model[-1].predict(comp_clk_est_time[-1].reshape(1, -1))[
            0
        ]
