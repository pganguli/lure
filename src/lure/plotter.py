...
class Plotter:
                ...
                for x_val, sim_results in series_result.items():
                    ...
                    for sim_result in sim_results:
                        ...
                        for i in range(num_sim_data):
                            min_time = np.inf
                            for node in range(len(sim_data)):
                                if (sim_data_indices[node] < len(sim_data[node])) and (
                                    sim_times[node][sim_data_indices[node]] < min_time
                                ):
                        ...
                        axes_list[trial].set_title(
                            f"Trial {trial}", loc="left", fontdict={"fontsize": "small"}
                        )
                        trial += 1
                    plt.xlabel("Time (ms)")
                    plt.tight_layout()
                    plt.savefig(
                        f"{self.save_fig_dir}{series_key}_xval{x_val}.{self.extension}"
                    )
                    plt.close()
        ...
        for exp_result in self.results:
            for series_key, series_result in exp_result.items():
                for sim_result in series_result.sim_results[xval]:
                    ...
                    plt.tight_layout()
                    plt.savefig(
                        f"{self.save_fig_dir}/time_series_{series_key}_x{xval}_nids{node_ids}_seed{sim_result.seed}_timerange{time_range}.{self.extension}"
                    )
                    plt.close()

    def plot_stat_distributions(
        self, stat_name: StatName, xval: Hashable = None, postfix: str = ""
    ):
        ...
        for exp_result in self.results:
            for series_key, series_results in exp_result.items():
                ...
                plt.hist(vals)
                plt.savefig(
                    f"{self.save_fig_dir}/{stat_name}_{series_key}_{postfix.replace('.', '-')}.pdf"
                )
                plt.close()
                        ...
                        if vals:
                            ...
                            if stat_name is StatType.PACKET_ARRIVAL_INTERVALS:
                                ...
                                if rate:
                                    ...
                                    plt.plot(
                                        x,
                                        expon.pdf(x, scale=1 / rate),
                                        label="expon pdf",
                                    )
                                else:
                                    print(
                                        "Trying to plot ideal distribution but no traffic rate found."
                                    )
                            plt.savefig(
                                f"{self.save_fig_dir}/{stat_name}_{series_key}_n{s.get(StatType.NODE_ID)}.{self.extension}"
                            )
                            plt.close()
    ...
    def plot_y_vs_x(
        ...
        if data_type not in data_types:
            ...
            return
        elif (data_types == "percentile") and (
            xth_percentile < 1 or xth_percentile > 99
        ):
            print(
                "Plotter: percentile plots must specify the percentile in the xth_percentile parameter"
            )
            return
            ...
            for exp_index, exp_result in enumerate(self.results):
                ...
                for series_key, series_config in self.config.series.items():
                    ...
                    for x_val, sim_results in series_result.items():
                        ...
                        for sim_result in sim_results:
                            if (ytype == "delay" or ytype == "active_delay") or (
                                ytype == "all"
                                and (
                                    ytypes[plot_index] == "delay"
                                    or ytypes[plot_index] == "active_delay"
                                )
                            ):
                                ...
                                if ytype == "active_delay" or (
                                    ytype == "all"
                                    and ytypes[plot_index] == "active_delay"
                                ):
                                    active = True
                                else:
                                    active = False
                                ...
                                for node, s in enumerate(sim_result):
                                    ...
                                    if (
                                        active
                                    ):  # Determine action based off of delay type
                                        node_data = (
                                            StatsParser.single_hop_active_delays(s)
                                        )
                                    else:
                                ...
                                # combine delays
                                combined_delays = StatsParser.combine_node_data(
                                    sim_data=delays, sim_times=pkt_sent_times
                                )

                                # use subset of data at stabilization point
                                if skips_for_stabilization > 0:
                                    combined_delays = combined_delays[
                                        skips_for_stabilization : len(combined_delays)
                                    ]
                            ...
                            elif ytype == "throughput" or (
                                ytype == "all" and ytypes[plot_index] == "throughput"
                            ):
                                if skips_for_stabilization == 0:
                                    throughput = StatsParser.total_throughput_bps(
                                        sim_result
                                    )
                                else:
                                    throughput = (
                                        StatsParser.total_steady_throughput_bps(
                                            sim_result, skips_for_stabilization
                                        )
                                    )
                                ...
                                ytype == "all" and ytypes[plot_index] == "count"
                            ):
                                counts = []
                                for s in sim_result:
                        ...
                        if len(y_datapoints) == 0:
                            ...
                            # plot 95th percentil
                                ...
                                y_to_graph.append(
                                    np.percentile(y_datapoints, xth_percentile)
                                )
                    ...
                    if ytype == "delay" or (
                        ytype == "all" and ytypes[plot_index] == "delay"
                    ):
                        y_to_graph = [
                            y_to_graph[i] / 1000 for i in range(len(y_to_graph))
                        ]  # convert from ms to s
                    elif ytype == "active_delay" or (
                        ytype == "all" and ytypes[plot_index] == "active_delay"
                    ):
                        y_to_graph = [
                            y_to_graph[i] / 1000 for i in range(len(y_to_graph))
                        ]  # convert from ms to s
                        ...
                        y_to_graph = [
                            y_to_graph[i] for i in range(len(y_to_graph))
                        ]  # convert from pkts/ms to bps, 1 pkt = 32bit payload
                    else:
                        # This should never happen because we filter these out at the beginning
                        pass
                    ...
                    if show_ci:
                        if (ytype == "delay" or ytype == "active_delay") or (
                            ...
                            shading_values = [
                                shading_values[i] / 1000 for i in range(len(y_to_graph))
                            ]  # convert from ms to s
                        elif ytype == "throughput" or (
                            ytype == "all" and ytypes[plot_index] == "throughput"
                        ):
                            shading_values = [
                                shading_values[i] for i in range(len(y_to_graph))
                            ]  # convert from pkts/ms to bps, 1 pkt = 32bit payload
                        else:
                            # This should never happen because we filter these out at the beginning
                            pass
                    y_lower_to_graph = [
                        max((y_to_graph[y_index] - shading_values[y_index]), 0)
                        for y_index in range(len(y_to_graph))
                    ]
                    y_upper_to_graph = [
                        max((y_to_graph[y_index] + shading_values[y_index]), 0)
                        for y_index in range(len(y_to_graph))
                ...
                if plotted_bounds == 0:
                    ...
                    plot_upper_bound = (
                        self.config.plot_upper_bounds
                    )  # 0 = don't plot, 1 = plot
                    if plot_upper_bound == 1:
                        bounds = plotter_type.plot_upper_bounds(
                            self, ytype, x_to_graph, exp_result
                        )
                    # plot vertical line at intersection of traffic and lifecycle upper bounds
                    plot_intersection_bool = (
                        self.config.plot_bound_intersection_vert
                    )  # 0 = don't plot, 1 = plot
                ...
                plotter_type.label_plot(
                    exp_result,
                    ytype,
                    num_simulations,
                    data_type=data_type,
                    percentile=xth_percentile,
                )
                fontsize = 16
                ...
                plotter_type.save_plot(
                    self,
                    exp_index,
                    ytype,
                    skips_for_stabilization,
                    data_type=data_type,
                    percentile=xth_percentile,
                )
                ...
                for k, v in final_vals.items():
                    ...
                    try:
                        ...
                        print(
                            f"\tNormalized to {self.config.config['baseline']} - {k}: {normalized_vals}"
                        )
                    except KeyError:
                        print(
                            'No baseline for normalization found. Configure key "baseline" in plot.json.'
                        )
    ...
    def plot_time_series_interactive(
        ...
        if series_index is not None:
            series_result = ResultsParser.getSeriesResultByIndex(
                exp_result, series_index
            )
        elif series_metadatakey is not None:
            series_result = ResultsParser.getSeriesResultByMetadataKey(
                exp_result, series_metadatakey
            )
        else:
        ...
        plt.subplots_adjust(bottom=0.2, right=0.8, hspace=0.3)
        try:
            axes_list[0]
        except:
            axes_list = [axes_list]
        ...
        for node_index, s in enumerate(stats_list):
            ...
            for i, name in enumerate(ts_names):
                ...
                axes_list[i].plot(
                    xs,
                    ys,
                    ls=linestyles[node_index % (len(linestyles))],
    ...
    def plot_vert_intersection(self, line1_ys, line2_ys, xs):
        ...
        if (x_intersection_pt > 0) and (x_intersection_pt <= float(xs[len(xs) - 1])):
            ...
            y_for_vert.append(0)
            y_for_vert.append(
                line1_ys[len(line1_ys) - 1]
            )  # line1_ys is likely the max y value on the graph
    ...
    def plot_vs_x_y(
        ...
        if not self.results or len(self.results.experiment_results) <= 1:
            self.debug(
                "Plotter: plot_vs_x_y: Not enough dimensions - did you mean to use plot_vs_x instead?"
            )
            return

        xs = list(self.results.experiment_results[0].series_results.values())[
            0
        ].x_values
        ys = xs
        # if ytype == 'extratime':
                        ...
                        if ztype == "delay":
                            zs[x][y][p].extend(
                                np.divide(
                                    [
                                        t[0]
                                        for t in StatsParser.time_to_handshakes(
                                            [stats_list[0]]
                                        )
                                    ],
                                    1000.0,
                            ...
                            zs[x][y][p].append(
                                StatsParser.total_throughput_bps(stats_list)
                            )
                        elif ztype == "lifecycle":
                            zs[x][y][p].append(
                                stats_list[0].get(StatType.NODE_LIFECYCLE_RATIO)
                            )
        ...
        if surface3d:
            ...
            plt.setp(
                ax.get_yticklabels(), ha="right", rotation_mode="anchor", fontsize=20
            )

            # Loop over data dimensions and create text annotations.
            for i in range(len(Y)):
                for j in range(len(X)):
                    ax.text(j, i, Z[i, j], ha="center", va="center", color="w")
        ...
        plt.savefig(
            self.save_fig_dir
            + "avg_%s_vs_%s_%s_3d.%s"
            % (ztype, xtype, postfix.replace(".", "-"), self.extension)
        )
        plt.close()
    ...
    def plot_cdf(
        ...
        xtype: str = "delay",
        xval: Hashable = None,
        skips_for_stabilization: int = 0,
        legend_placement: str = "best",
    ):
        ...
        for exp_index, exp_result in enumerate(self.results):
            for series_metadata, series_result in exp_result.items():
                ...
                for sim_result in sim_results:
                    for stats in sim_result:
                        if xtype == "delay":
                            stats.get(StatType.PACKETS_GENERATED)
                            delays = []
                            try:
                                ...
                                print(
                                    f"Zero packets for a node in series {series_metadata.key} at {xval}"
                                )
                            plt.xlabel("Delay (s)")
            ...
            plt.xlim(0, 1400)
            plt.savefig(
                f"{self.save_fig_dir}/{xtype}_{xval}_cdf_{exp_index}.{self.extension}"
            )
            plt.close()
