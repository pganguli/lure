...
class Config:
    ...
    @staticmethod
    def get_dict_permutations(
        d: dict, root: dict = None, keys: List = None
    ) -> Dict[Any, dict]:
    ...
    def extract(self, val: str, obj: object, default: Any):
        ...
        try:
            ...
            print(
                f"Warning: config key {val} not found in instance of {self.__class__}. Using default value of {default}."
            )
    ...
    def merge_type_file(self, type: str) -> bool:
        ...
        if type == "default":
            print(
                f'Warning: using built-in default type for {self.CONFIG_NAME}. If you meant to load a custom type, name it something other than "default".'
            )
            return False
        if self.CONFIG_NAME:
    ...
    def merge_default_config(self):
        ...
        if not self.CONFIG_NAME:
            return
        s = pkgutil.get_data(
            f"{__package__}.default", f"default_{self.CONFIG_NAME}.json"
        )
        if s:
            self.merge_dict(json.loads(s))
        else:
            print(
                f'Warning: default json not found for config name "{self.CONFIG_NAME}."'
            )
...
class LoggerConfig(Config):
    ...
    def merge_type_file(self, type: str):
        ...
        if not custom_file_found:
            print(
                "Warning: custom logger type file not found, trying built-in loggers."
            )
            s = pkgutil.get_data(
                f"{__package__}.default", f"{type}_{self.CONFIG_NAME}.json"
            )
            if s:
                self.merge_dict(json.loads(s))
    ...
    def get_level(self, component: str) -> LogLevel:
        ...
        try:
            ...
            print(
                f"Warning: no level found for {component} in logger config. Using INFO."
            )
            lvl = "info"
        return LogLevel(lvl)
    ...
    def __init__(
        ...
        if len(self.config["nodes"]) > 0:
            if self.config["num_nodes"]:
                print(
                    "Warning: explicit node list overrides use of num_nodes and template_node."
                )
            if "logger" in self.config["nodes"][0]:
                self.nodes = [SensorNodeConfig(n) for n in self.config["nodes"]]
            else:
                self.nodes = [
                    SensorNodeConfig(n, logger_config, stats_config)
            ...
            self.nodes = [
                SensorNodeConfig(
                    self.config["template_node"], logger_config, stats_config
                )
...
class ExperimentConfig(Config):
    ...
    def __init__(
        ...
        super().__init__(config)
        self.energy_model: EnergyModelConfig = EnergyModelConfig(
            self.config["energy_model"]
        )
        self.logger: LoggerConfig = logger_config
        self.stats: StatsConfig = stats_config
        self.series = [
            DataSeriesConfig(s, logger_config, stats_config)
        ...
        if len(series_key_set) != len(self.series):
            print(
                f"Warning: non-unique series keys detected ({series_key_set} vs {[s.config['key'] for s in self.series]})! Results may be overwritten!"
            )
...
class LureConfig(Config):
    ...
    def __init__(self, config_dir: str, top_config_file: str):
        ...
        if "logger" in self.config:
            ...
            logger = None
        self.experiments = [
            ExperimentConfig(e, logger, stats) for e in self.config["experiments"]
        ]
...
class PlotterConfig(Config):
    ...
    def __init__(self, config_dir: str = "config"):
        ...
        try:
            ...
            self.plot_bound_intersection_vert: int = self.config[
                "plot_bound_intersection_vert"
            ]
