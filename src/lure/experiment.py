...
from lure.config.configuration import (
    Config,
    DataSeriesConfig,
    ExperimentConfig,
    LoggerConfig,
...
class DataSeries:
    ...
    def __init__(
        self, config: DataSeriesConfig, energy_model_config: EnergyModelConfig
    ):
        self.plot_config = None
        self.key = None
        ...
        if series_len > 0:
            print(
                "Info: found independent variables for SensorNode (and will not look in EnergyModel)."
            )
            ...
            for i, node_perms in enumerate(self.nodes):
                if len(node_perms) < series_len:
                    print(
                        f"Warning: inconsistent independent var lengths found ({series_len} and {len(node_perms)}). Adding some filler nodes."
                    )
                    for _ in range(series_len - len(node_perms)):
                        self.nodes[i].append(
                            (None, copy.deepcopy(self.nodes[i][-1][1]))
                        )
            ...
            energy_model_dicts = Config.get_dict_permutations(
                energy_model_config.config
            )
            if energy_model_dicts:
                for ivar_val, d in energy_model_dicts.items():
                    self.energy_models[ivar_val] = Config.instantiate_from_dict(
                        d, "lure.energy"
                    )
        ...
        if not self.nodes and not self.energy_models:
            print(
                'Info: no independent variables detected for SensorNode or EnergyModel. XValue in SeriesResult will be "None".'
            )
            for node in other_nodes_list:
                self.nodes.append([(None, node)])
            # self.nodes["Unknown"] = other_nodes_list
            self.energy_models[None] = Config.instantiate_from_dict(
                energy_model_config.config, "lure.energy"
            )
            self.x_values.append(None)
        elif not self.energy_models:
            self.energy_models = {
                k: Config.instantiate_from_dict(
                    energy_model_config.config, "lure.energy"
                )
...
class Experiment:
    ...
    def run_series(
        self, series: DataSeries, pool: Pool = None, progress_bar=None
    ) -> SeriesResult:
