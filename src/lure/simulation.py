...
class Simulation(Loggable, StatsObserver):
    ...
    def __init__(
        ...
        self.logger = LureLogger(
            logger_config, self.simpy_env, output_dir=self.output_dir
        )
        self.start_time = None

        self.register_log(
            self.logger, logger_config.get_level("simulation"), "Simulation"
        )

        self.energy_model = energy_model
        self.energy_model.register_log(
            self.logger, logger_config.get_level("energy_model"), "EnergyModel"
        )
        ...
        for n in self.nodes:
            ...
            n.process = self.simpy_env.process(
                n.execute(self.simpy_env, self.energy_model)
            )
    ...
    def on_update(self, name: StatName, val_change: Union[int, float], stats: Stats):
        ...
        """
        if (
            name == StatType.PACKETS_RECEIVED_APP
            and self.max_packets
            and self.max_packets > 0
        ):
            ...
            self.debug(
                f"{self.packets_so_far} packets sent app, real_time={elapsed}, seconds={cur_time}"
            )
