...
class LambdaPlot(PlotterType):
    ...
    def x_axis_vals(self, ytype=None, x_to_graph=None):
        if ytype == "delay":
            ...
            print(
                "Lambda.x_axis_vals: unrecognized ytype: %s OR bad x_to_graph: %s"
                % (ytype, x_to_graph)
            )
            return
        return x_to_graph
    ...
    def plot_upper_bounds(
        self, plotter=None, ytype=None, x_to_graph=None, experiment=None
    ):
        bounds = []
        if ytype == "throughput":
            ...
            sim_result = list(
                list(experiment.series_results.values())[0].sim_results.values()
            )[0][0]
            lcrs = []
            for n in sim_result:
            ...
            if lcrs[0]:
                ...
                lifecycle_bound = []
                slot_size = 5
                lifecycle_bound = [min_lifecycle * (1000 / slot_size) * 32] * len(
                    x_to_graph
                )
                plt.plot(
                    x_to_graph,
            ...
            print(
                "Lambda.upper_bounds: unrecognized ytype: %s OR bad x_to_graph: %s"
                % (ytype, x_to_graph)
            )
            return
        return bounds
    ...
    def save_plot(
        ...
        sim_result = list(
            list(plotter.results.experiment_results[exp_index].series_results.values())[
                0
            ].sim_results.values()
        )[0][0]
        for n in sim_result:
        ...
        plt.savefig(
            f"{plotter.save_fig_dir}{exp_index}_{title_prefix}_{ytype}_vs_traffic_{lifecycle_ratio}lifecycle_{skips_for_stabilization}skipped.{plotter.extension}"
        )
        # if ytype == 'delay':
