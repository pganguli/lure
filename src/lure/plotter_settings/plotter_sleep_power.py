...
class SleepPowerPlot(PlotterType):
    ...
    def x_axis_vals(self, ytype=None, x_to_graph=None):
        return np.multiply(x_to_graph, 1000)

    def plot_upper_bounds(self, plotter=None, ytype=None, x_to_graph=None, experiment: ExperimentResult=None):
        bounds = []
        if ytype == 'throughput': 
            ...
            plt.plot(x_to_graph, traffic_bound, label=f'Traffic Upper Bound', linewidth=3, ls='dotted', zorder=0, color='black')
            ...
            print('Charging.upper_bound: unrecognized ytype: %s OR bad x_to_graph: %s' % (ytype, x_to_graph))
            return
    ...
    def label_plot(self, experiment=None, ytype=None, num_simulations=0, data_type=None, percentile=None):
        ...
        if ytype == 'delay':
            ...
            plt.xlabel('Sleep Power (mW)', fontsize=fontsize)
            #plt.ylim(1,10000)
    ...
    def save_plot(self, plotter=None, exp_index=0, ytype=None, skips_for_stabilization=0, data_type=None, percentile=None):
        ...
        plt.savefig(f'{plotter.save_fig_dir}{exp_index}_{title_prefix}_{ytype}_vs_sleep_power_{traffic_rate}lambda_{skips_for_stabilization}skipped.{plotter.extension}')
