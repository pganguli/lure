...
class SleepPowerPlot(PlotterType):
    ...
    def plot_charging_regions(self):
        ...
        plt.plot(
            [operating_discharge_rate + operating_discharge_rate * coeff] * 2,
            plt.axes().get_ylim(),
            linewidth=1.5,
            ls="dotted",
            zorder=0,
    ...
    def save_plot(
        ...
        plt.savefig(
            f"{plotter.save_fig_dir}{exp_index}_{title_prefix}_{ytype}_vs_sleep_power_{traffic_rate}lambda_{skips_for_stabilization}skipped.{plotter.extension}"
        )
